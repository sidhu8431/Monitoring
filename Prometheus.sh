...
install_prometheus() {
    ...
    if [ -f "$PROM_DIR/prometheus.yml" ]; then
        echo "[*] Existing prometheus.yml found. Preserving it."
    else
        echo "[!] Warning: prometheus.yml not found in $PROM_DIR."
        echo "    Please ensure a valid config file exists before starting Prometheus."
    fi
